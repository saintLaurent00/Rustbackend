...
// The RlsRuleFunc type is an adapter to allow the use of ordinary
// function as RlsRule mutator.
type RlsRuleFunc func(context.Context, *ent.RlsRuleMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RlsRuleFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.RlsRuleMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RlsRuleMutation", m)
}
